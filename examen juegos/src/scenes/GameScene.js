export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.player = this.physics.add.sprite(startPoint.x + startPoint.width / 2, startPoint.y + startPoint.height / 2, 'player', 0);
    ...
    const exit = this.add.rectangle(exitPoint.x + exitPoint.width / 2, exitPoint.y + exitPoint.height / 2, 64, 64, 0x22cc22).setOrigin(0.5);
    this.physics.add.existing(exit, true);
    this.exit = exit;
    console.log('Exit created at:', exitPoint.x + exitPoint.width / 2, exitPoint.y + exitPoint.height / 2);
    ...
    objectLayer.objects.forEach((obj) => {
      if (obj.name === 'Gem') {
        const gem = this.gems.create(obj.x + (obj.width || 0) / 2, obj.y + (obj.height || 0) / 2, 'gem');
        gem.setScale(0.7);
        gem.body.setCircle(12);
      }

      if (obj.name === 'Hazard') {
        const hazard = this.hazards.create(obj.x + (obj.width || 0) / 2, obj.y + (obj.height || 0) / 2, 'hazard');
        hazard.setScale(1.1);
        hazard.refreshBody();
      }

      if (obj.name === 'Enemy') {
        const enemy = this.enemies.create(obj.x + (obj.width || 0) / 2, obj.y + (obj.height || 0) / 2, 'enemy');
    ...
    this.hudLives = this.add.text(16, 46, 'Vidas: 3', {
      fontSize: '20px',
      fill: '#ffffff',
      backgroundColor: 'rgba(0,0,0,0.5)',
      padding: { x: 10, y: 6 }
    ...
    this.hudTime = this.add.text(16, 76, 'Tiempo: 90', {
      fontSize: '20px',
      fill: '#ffffff',
      backgroundColor: 'rgba(0,0,0,0.5)',
      padding: { x: 10, y: 6 }
    ...
    this.destinationText = this.add.text(16, 110, 'Llega a la salida después de recolectar las gemas', {
      fontSize: '16px',
      fill: '#88ff88',
      backgroundColor: 'rgba(0,0,0,0.5)',
      padding: { x: 10, y: 6 }
  ...
  collectGem(player, gem) {
    ...
    if (this.score >= 5) {
      this.destinationText.setText('¡Excelente! Ahora ve hacia la salida verde.');
    }
  }
